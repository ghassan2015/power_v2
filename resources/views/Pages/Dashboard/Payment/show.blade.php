                        ...
                        <div class="col-md-9">
                            ...
                            <div class="d-flex justify-content-between text-white pt-6">
                                <div class="d-flex flex-column flex-root">
                                    <span class="font-weight-bolde mb-2r">تاريخ الدفعة</span>
                                    <span class="opacity-70">{{ $Payment->created_at->format('M-m')}}</span>
                                </div>
                                <div class="d-flex flex-column flex-root">
                                    <span class="font-weight-bolder mb-2">رقم الفاتورة:</span>
                                    <span class="opacity-70">{{ $Payment->Invoice->Customer->full_name.'/'.$Payment->created_at->format('M-m')}}</span>
                                </div>
                                <div class="d-flex flex-column flex-root">
                                    <span class="font-weight-bolder mb-2">اسم المشترك</span>
                                    <span class="opacity-70">{{ $Payment->Invoice->Customer->full_name}}
                                    </span>
                                </div>
                                <div class="d-flex flex-column flex-root">
                                    <span class="font-weight-bolder mb-2">العنوان</span>
                                    <span class="opacity-70">{{$Payment->Invoice->Customer->location}}
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tr class="font-weight-boldest font-size-lg">
                                        <td class="pl-0 pt-7">{{$Payment->Invoice->Customer->full_name}}</td>
                                        <td class="text-right pt-7">{{$Payment->Invoice->Customer->kw_price}}</td>
                                        <td class="text-right pt-7">{{($Payment->Invoice->current_reading)-($Payment->Invoice->previous_reading)}}</td>
                                        <td class="text-danger pr-0 pt-7 text-right">{{$Payment->Invoice->total_price}}
                                            <span><i class="fas fa-shekel-sign" style="color: #000000 ;font-size: 12px"></i></span></td>
                                        <td class="text-right pt-7">{{($Payment->Invoice->total_price)-($Payment->Invoice->remaining)}}    <span><i class="fas fa-shekel-sign" style="color: #000000 ;font-size: 12px"></i></span></td>

                                        <td class="text-danger pr-0 pt-7 text-right">

                                            @if($Payment->Invoice->status==2)
